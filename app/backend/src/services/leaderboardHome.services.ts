...
export default class LeaderboardHome {
  static calculatePointVitoriaEmpates(time: Iteams, matches: IMatchesTime[]) {
    ...
    const calculateDerrotasGolsJogos = LeaderboardHome.calculateDerrotasGols(time, matches);
  ...
  static async leaderboardHomeFunc() {
    ...
    result.forEach((time) => {
      const res = LeaderboardHome.calculatePointVitoriaEmpates(time, matches);
      const eff = (res.point / (res.calculateDerrotasGolsJogos.jogos * 3)) * 100;
      obj = [
        ...obj,
        LeaderboardHome.createObj(res, eff, time),
      ];
    });

    return LeaderboardHome.sort(obj);
  }
}
